...
export function buildGraph(document: SbomDocument): ParsedSbom {
  ...
  // Index components by bom-ref and name
  ...
  for (const component of document.components) {
    const bomRef = component['bom-ref'];
    const hasDependents = reverseDeps.has(bomRef) && reverseDeps.get(bomRef)!.length > 0;

    if (!hasDependents && isMendixMarketplaceComponent(component)) {
...
export function traceToRoot(graph: ParsedSbom, bomRef: string): TraceResult {
  ...
  while (queue.length > 0 && iterations < maxIterations) {
    ...
    if (isRoot) {
      ...
      const components = current.path.map(ref => graph.componentsByRef.get(ref)!);
      paths.push({
        components,
        isComplete: true,
      });
